...
def main(params):
    ...
    imgs_dir = os.path.join(params.output_dir, 'imgs_coco')
    ...
    watermarking_loader = utils.get_dataloader(params.val_dir, vqgan_transform, params.batch_size, num_imgs=1000, shuffle=True, num_workers=4, collate_fn=None)
    ...
    for ii_key in range(params.num_keys):
        ...
        print('\n')
    watermarking(watermarking_loader, ldm_ae, ldm_decoder, msg_decoder, vqgan_to_imnet, key, params)
...
def train(data_loader: Iterable, optimizer: torch.optim.Optimizer, loss_w: Callable, loss_i: Callable, ldm_ae: AutoencoderKL, ldm_decoder:AutoencoderKL, msg_decoder: nn.Module, vqgan_to_imnet:nn.Module, key: torch.Tensor, params: argparse.Namespace):
    ...
    for ii, imgs in enumerate(metric_logger.log_every(data_loader, params.log_freq, header)):
        ...
        log_stats = {
            ...
            # "psnr_ori": utils_img.psnr(imgs_w, imgs_coco).mean().item(),
...
@torch.no_grad()
def watermarking(data_loader: Iterable, ldm_ae: AutoencoderKL, ldm_decoder: AutoencoderKL, msg_decoder: nn.Module,
    ...
    for ii, imgs in enumerate(data_loader):
        ...
        imgs_z = imgs_z.mode()
        imgs_w = ldm_decoder.decode(imgs_z)  # b z h/f w/f -> b c h w
        path = params.imgs_dir+"val/watermarked/"
        os.makedirs(path, exist_ok=True)




        img_w = torch.clamp(utils_img.unnormalize_vqgan(imgs_w), 0, 1)

        for i, img in enumerate(img_w):

            image_pil = to_pil(img.cpu())
            image_pil.save(os.path.join(path, f'{ii:04}{i:04}.png'), format='PNG')
